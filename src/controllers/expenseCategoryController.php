...
function createExpenseCategory() {
    ...
    $stmt = db()->prepare("SELECT id FROM expense_categories WHERE user_id = ? AND name = ?");
    $stmt->execute([$userId, trim($input['name'])]);
    if ($stmt->fetch()) {
        json(['status' => 'error', 'message' => 'A category with that name already exists'], 409);
    }

    $stmt = db()->prepare("INSERT INTO expense_categories (user_id, name) VALUES (?, ?)");
...
function updateExpenseCategory($id) {
    ...
    $stmt = db()->prepare("SELECT id FROM expense_categories WHERE id = ? AND user_id = ?");
    ...
    $stmt = db()->prepare("SELECT id FROM expense_categories WHERE user_id = ? AND name = ? AND id != ?");
    $stmt->execute([$userId, trim($input['name']), $id]);
    if ($stmt->fetch()) {
        json(['status' => 'error', 'message' => 'A category with that name already exists'], 409);
    }

    $stmt = db()->prepare("UPDATE expense_categories SET name = ? WHERE id = ? AND user_id = ?");
